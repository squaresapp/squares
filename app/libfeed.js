...
(function (Libfeed) {
    ...
    Libfeed.ForeignDocumentReader = ForeignDocumentReader;
})(Libfeed || (Libfeed = {}));
var Libfeed;
(function (Libfeed) {
    /**
    ...
    class ForeignDocumentSanitizer {
        ...
        /** */
        read() {
            const reader = new Libfeed.ForeignDocumentReader(this.rawDocument);
        ...
        resolvePlainUrl(plainUrl) {
            if (plainUrl.startsWith("data:") ||
                ...
                /^[a-z\-]+:/g.test(plainUrl))
                return plainUrl;
            return Libfeed.Url.resolve(plainUrl, this.baseHref);
        }
        /** */
    ...
    Libfeed.ForeignDocumentSanitizer = ForeignDocumentSanitizer;
    /** */
...
(function (Libfeed) {
    ...
    (function (History) {
        ...
        const stack = [];
        let stackPosition = -1;
    })(History = Libfeed.History || (Libfeed.History = {}));
})(Libfeed || (Libfeed = {}));
var Libfeed;
(function (Libfeed) {
    /**
        ...
        const defaultOptions = {
            getPoster: index => {
                ...
                return new Promise(async (resolve) => {
                    ...
                    resolve(poster || Libfeed.getErrorPoster());
                });
            },
            fillBody: async (fillElement, selectedElement, index) => {
                ...
                if (!reel)
                    return selectedElement.append(Libfeed.getErrorPoster());
                fillElement.append(Libfeed.getSandboxedElement([...reel.head, ...reel.sections], reel.url));
            }
        };
    ...
    Libfeed.getOmniviewFromFeed = getOmniviewFromFeed;
})(Libfeed || (Libfeed = {}));
var Libfeed;
(function (Libfeed) {
    /**
    ...
    function getRemoteSectionElements(container = document.body) {
        return Libfeed.getElements("SECTION[src], SECTION[data-src]", container);
    }
    Libfeed.getRemoteSectionElements = getRemoteSectionElements;
    /**
     * Returns a fully-qualified version of the URI specified as the source
     * of the content in a <section> element.
     */
    function getRemoteSectionSource(section, documentUrl = Libfeed.Url.getCurrent()) {
        const src = section.getAttribute("src") || section.getAttribute("data-src") || "";
        return src ? Libfeed.Url.resolve(src, documentUrl) : "";
    }
    Libfeed.getRemoteSectionSource = getRemoteSectionSource;
    /**
     * Loads the content of any remote <section> elements
     * defined within the specified container element.
     */
    async function resolveRemoteSections(container = document, documentUrl = Libfeed.Url.getCurrent()) {
    ...
    Libfeed.resolveRemoteSections = resolveRemoteSections;
})(Libfeed || (Libfeed = {}));
var Libfeed;
(function (Libfeed) {
    Libfeed.standardCss = `
    ...
    function getStandardCss() {
        ...
        style.textContent = Libfeed.standardCss;
        return style;
    }
    Libfeed.getStandardCss = getStandardCss;
})(Libfeed || (Libfeed = {}));
var Libfeed;
(function (Libfeed) {
    /**
...
(function (Libfeed) {
    ...
    function getSandboxedElement(contents, baseUrl) {
        ...
        const head = [Libfeed.getStandardCss()];
        const body = [];
        ...
        baseUrl = Libfeed.Url.folderOf(baseUrl);
        convertEmbeddedUrlsToAbsolute(shadow, baseUrl);
        return container;
    }
    Libfeed.getSandboxedElement = getSandboxedElement;
    /**
     *
     */
    function convertEmbeddedUrlsToAbsolute(parent, baseUrl) {
        ...
        for (const element of elements) {
            ...
            for (const attribute of attrs)
                attribute.value = Libfeed.Url.resolve(attribute.value, baseUrl);
            for (const p of cssPropertiesWithUrls) {
                ...
                pv = pv.replace(/\burl\(".+?"\)/, substr => {
                    ...
                    const url = Libfeed.Url.resolve(unwrapUrl, baseUrl);
                    return `url("${url}")`;
                });
    ...
    async function getPageFromUrl(url) {
        const baseUrl = Libfeed.Url.folderOf(url);
    ...
    Libfeed.getPageFromUrl = getPageFromUrl;
    /**
    ...
    Libfeed.getFeedsFromDocument = getFeedsFromDocument;
    /**
    ...
    Libfeed.getDocumentFromUrl = getDocumentFromUrl;
    /**
    ...
    async function sanitizeDocument(url, html) {
        const docUri = Libfeed.Url.folderOf(url);
        const sanitizer = new Libfeed.ForeignDocumentSanitizer(html, docUri);
    ...
    Libfeed.handleSubmitFailure = handleSubmitFailure;
    ...
    function getFeedUrl(within = document) {
        ...
        return href ? Libfeed.Url.resolve(href, Libfeed.Url.getCurrent()) : "";
    }
    Libfeed.getFeedUrl = getFeedUrl;
    /**
    ...
    async function getFeedUrls(feedUrl) {
        ...
        if (type !== "text/plain") {
            ...
            urls.push(...fetchResult.text
                ...
                .map(s => Libfeed.Url.resolve(s, Libfeed.Url.folderOf(feedUrl))));
            bytesRead = fetchResult.text.length || 0;
        }
        return urls;
    }
    Libfeed.getFeedUrls = getFeedUrls;
    /**
    ...
    async function getFeedMetaData(feedUrl) {
        let currentUrl = Libfeed.Url.folderOf(feedUrl);
        let author = "";
        let description = "";
        let icon = "";
        for (let safety = 1000; safety-- > 0;) {
            ...
            if (httpContent) {
                ...
                const reader = new Libfeed.ForeignDocumentReader(htmlContent);
    ...
    Libfeed.getFeedMetaData = getFeedMetaData;
    /**
    ...
    Libfeed.getPosterFromUrl = getPosterFromUrl;
    /**
    ...
    Libfeed.getPostersFromFeed = getPostersFromFeed;
    /**
    ...
    function getEmbeddedOmniviewFromFeed(urls, omniviewOptions = {}) {
        if (typeof Omniview === "undefined")
            throw new Error("Omniview library not found.");
        const raw = new Raw();
        const omniview = Libfeed.getOmniviewFromFeed(urls, omniviewOptions);
    ...
    Libfeed.getEmbeddedOmniviewFromFeed = getEmbeddedOmniviewFromFeed;
    /**
    ...
    Libfeed.getErrorPoster = getErrorPoster;
    //# Generic
    /**
    ...
    async function getHttpContent(relativeUri, options = {}) {
        relativeUri = Libfeed.Url.resolve(relativeUri, Libfeed.Url.getCurrent());
        try {
    ...
    Libfeed.getHttpContent = getHttpContent;
    /**
    ...
    Libfeed.getElements = getElements;
})(Libfeed || (Libfeed = {}));
